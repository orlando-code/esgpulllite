...
from esgpulllite.constants import PROVIDERS
...
class Auth:
    ...
    @staticmethod
    def from_config(config: Config, credentials: Credentials = Credentials()) -> Auth:
        return Auth.from_path(config.paths.auth, credentials)

    @staticmethod
    def from_path(path: Path, credentials: Credentials = Credentials()) -> Auth:
