...
from esgpulllite.models import Options, Query, Tag
...
rest = pp.Group(
    comment | facet | variable_no_sqbr | variable_cmip5 | variable_cmip6 | otherwise
)[...]
        ...
        for path in paths:
            if path.is_file():
                ...
                if len(queries) == 1 and queries[0].sha == empty_query.sha:
                    esg.ui.print(f"Skipping empty query produced from {path.resolve()}")
                    continue
