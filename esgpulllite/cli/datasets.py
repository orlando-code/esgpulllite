...
from esgpulllite.cli.decorators import args, groups, opts
from esgpulllite.cli.utils import init_esgpull, valid_name_tag
from esgpulllite.models import FileStatus
...
def datasets(
    ...
    with esg.ui.logging("datasets", onraise=Abort):
        ...
        datasets: defaultdict[str, DatasetCounter] = defaultdict(DatasetCounter)
        ...
        if json or yaml:
            datasets_dict = {
                dataset_id: counts.asdict() for dataset_id, counts in datasets.items()
            }
            if json:
