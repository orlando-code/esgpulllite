...
from esgpulllite.cli.decorators import args, groups, opts  # Imports CLI decorators
...
from esgpulllite.exceptions import PageIndexError  # Imports custom exception
from esgpulllite.graph import Graph  # Imports Graph class for data representation
from esgpulllite.models import Query  # Imports Query model
from esgpulllite.tui import Verbosity  # Imports Verbosity enum for TUI
...
def search(  # Defines the search command function
    ...
    with esg.ui.logging(
        "search", onraise=Abort
    ):  # Sets up logging for the search command
        ...
        query = esg.insert_default_query(query)[
            0
        ]  # Inserts the default query if not disabled
        if show:  # If show option is enabled
            ...
            esg.ui.raise_maybe_record(
                Exit(0)
            )  # Exits after showing, possibly recording
        esg.graph.add(
            query, force=True
        )  # Adds the query to the graph, forcing if necessary
        query = esg.graph.expand(
            query.sha
        )  # Expands the query using its SHA from the graph
        ...
        max_hits = min(
            page_size, nb - offset
        )  # Calculates maximum hits for the current page
        ...
        )  # Generates a range of IDs for the current page
        if dry_run:  # If dry run option is enabled
            ...
            esg.ui.raise_maybe_record(
                Exit(0)
            )  # Exits after dry run, possibly recording
        if facets_hints:  # If facets hints option is enabled
            not_distrib_query = query << Query(
                options=dict(distrib=False)
            )  # Creates a query without distributed search
            ...
            esg.ui.print(
                list(facet_counts[0]), json=True
            )  # Prints facet counts as JSON
            esg.ui.raise_maybe_record(
                Exit(0)
            )  # Exits after showing hints, possibly recording
        ...
        if max_hits > 200 and not yes:  # If many hits and not auto-confirmed
            nb_req = (
                max_hits // esg.config.api.page_limit
            )  # Calculates number of requests
        ...
        if detail is not None:  # If detail level is specified
            ...
            if json:  # If JSON output is requested
                selections = [
                    q.selection.asdict() for q in queries
                ]  # Extracts selections as dicts
            ...
            esg.ui.raise_maybe_record(
                Exit(0)
            )  # Exits after showing details, possibly recording
        ...
        if json:  # If JSON output is requested
            esg.ui.print(
                [f.asdict() for f in results], json=True
            )  # Prints results as JSON
        elif yaml:  # If YAML output is requested
            esg.ui.print(
                [f.asdict() for f in results], yaml=True
            )  # Prints results as YAML
        else:  # Otherwise, print in default format
            f_or_d = (
                "file" if file else "dataset"
            )  # Determines if searching for files or datasets
            ...
            if results:  # If there are results
                ...
                unique_nodes = {
                    (r.master_id, r.data_node) for r in results
                }  # Gets unique master ID and data node pairs
                needs_data_node = len(unique_nodes) > len(
                    unique_ids
                )  # Checks if data node is needed for uniqueness
                docs = filter_keys(
                    results, ids=ids, data_node=needs_data_node
                )  # Filters keys for display
