...
from esgpulllite.cli.utils import get_queries, init_esgpull, valid_name_tag
from esgpulllite.context import HintsDict, ResultSearch
from esgpulllite.exceptions import UnsetOptionsError
from esgpulllite.models import File, FileStatus, Query
from esgpulllite.tui import Verbosity, logger
...
@click.option(
    "--subset-criteria",
    type=str,
    help="Subset criteria for filtering files (JSON format).",
)
def update(
    query_id: str | None,
    tag: str | None,
    ...
    with esg.ui.logging("update", onraise=Abort):
        ...
        queries = [
            query for query in queries if query.tracked and query.sha != "LEGACY"
        ]
        if not qfs:
            ...
            for file in qf.files:
                if file.sha not in shas:
                    if subset_criteria_dict and not all(
                        ...
                        for key, value in subset_criteria_dict.items()
                    ):
                        continue
