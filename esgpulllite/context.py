...
from esgpulllite.exceptions import SolrUnstableQueryError
from esgpulllite.models import Dataset, File, Query
from esgpulllite.tui import logger
from esgpulllite.utils import format_date_iso, index2url, sync
...
class Context:
    ...
    async def _fetch_one(self, result: RT) -> RT:
        ...
        async with self.semaphores[host]:
            ...
            try:
                ...
                result.json = json.loads(resp.content.decode(encoding="latin-1"))
    ...
    async def _fetch(self, *in_results: RT) -> AsyncIterator[RT]:
        tasks = [asyncio.create_task(self._fetch_one(result)) for result in in_results]
        excs = []
        for task in tasks:
            result = await task
            yield result
