...
from esgpulllite.exceptions import (
    GraphWithoutDatabase,
    QueryDuplicate,
    TooShortKeyError,
)
from esgpulllite.models import Facet, Query, QueryDict, Tag, sql
from esgpulllite.models.utils import rich_measure_impl
...
class Graph:
    ...
    @staticmethod
    def _expand_name(name: str, shas: set[str], name_sha: dict[str, str]) -> str:
        if name in name_sha:
            sha = name_sha[name]
        elif name in shas:
            sha = name
    ...
    def add(
        ...
        *queries: Query,
        ...
        queue: list[Query] = [
            query.clone(compute_sha=True) if clone else query for query in queries
        ]
        ...
        new_name_sha = {
            name: shas[0] for name, shas in name_shas.items() if name not in skip_tags
        }
        if not force:
            for sha, query in new_queries.items():
                if query.require is not None:
                    sha = self._expand_name(query.require, new_shas, new_name_sha)
