...
from esgpulllite.models import Table
from esgpulllite.models.facet import Facet
from esgpulllite.models.file import FileStatus
from esgpulllite.models.query import File, Query, query_file_proxy, query_tag_proxy
from esgpulllite.models.selection import Selection, selection_facet_proxy
...
def count(item: Table) -> sa.Select[tuple[int]]:
    ...
    return sa.select(sa.func.count("*")).select_from(table).filter_by(sha=item.sha)
...
class file:
    ...
    def orphans() -> sa.Select[tuple[File]]:
        return sa.select(File).outerjoin(query_file_proxy).filter_by(file_sha=None)
    ...
    def shas_from_query(query_sha: str) -> sa.Select[tuple[str]]:
        return sa.select(query_file_proxy.c.file_sha).filter_by(query_sha=query_sha)
...
class selection:
    ...
    def orphans() -> sa.Select[tuple[Selection]]:
        return sa.select(Selection).outerjoin(Query).where(Query.sha == None)  # noqa
...
class tag:
    ...
    def orphans() -> sa.Select[tuple[Tag]]:
        return sa.select(Tag).outerjoin(query_tag_proxy).filter_by(tag_sha=None)
